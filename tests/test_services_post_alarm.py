...
async def test_list_and_clear_post_alarm_history_and_accept_revert(hass):
    ...
    coordinator._post_alarm_temp_overrides["switch.d1"] = {
        "applied": 300,
        "expires": 999999,
        "reason": "flipflop",
    }
    ...
    await hass.services.async_call(
        ...
        {"entry_id": entry.entry_id, "entity_id": "switch.d1"},
        blocking=True,
    )
    await asyncio.sleep(0)

    # Check entry.options updated
    assert entry.options.get("adaptive_mode_overrides") or entry.options.get(
    ...
    coordinator._post_alarm_corrections.append(
        {"entity": "switch.d2", "reason": "late_start"}
    )
    await hass.services.async_call(
        DOMAIN,
        "clear_post_alarm_history",
        {"entry_id": entry.entry_id, "entity_id": "switch.d1"},
        blocking=True,
    )
    # only switch.d2 should remain
    assert all(
        c.get("entity") != "switch.d1" for c in coordinator._post_alarm_corrections
    )

    # clear all
    coordinator._post_alarm_corrections.append(
        {"entity": "switch.d3", "reason": "drain_miss"}
    )
    await hass.services.async_call(
        DOMAIN, "clear_post_alarm_history", {"entry_id": entry.entry_id}, blocking=True
    )
